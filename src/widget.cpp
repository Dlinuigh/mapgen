...
//   // FIXME
//   没有正常找到邻接的集合加入，而是创建了多个邻接的集合，本来可以合并的
...
//   // for (auto it = adj_block[code].rbegin() + 1; it !=
//   adj_block[code].rend();
//   //      it++) {
...
// void Map::general_handle(SDL_Renderer *render, int idx, char code) {
...
//   // FIXME 还有问题,使用非fill的list不能被fill
//   // TODO 整理move point之类函数
//   // TODO 整理规范data的修改使用,draw的时机
//   /*
//   1.
//   data的存在必要性,使用一个字符数组达到迅速判断某一个点的邻接点的状态.代价就是维护
//   2.
//   draw与handle的分离是必要的可行的,draw的部分由维护的list向量提供,可以在O(n)的时间复杂度知道
//   3.
//   handle最重要的事情是:初始状态,状态转换,由于初始状态固定,所以只剩下状态切换,但是要求初始也是过程的输入
//   4.
//   频繁的访问点的邻接点,借助walked可以加速,避免那些点进入考虑范围内,本身如果是dfs也是需要判断的,只是它可以通过分支快速去除当然广度优先搜索更好了,如果可以早点剪除分支的话.能够加速的部分就是这些邻接点的快速的剪除本身就包含在里面的.所以使用4个if很必要.但是我在多个场合都有这部分.
//   5.
//   对于两种情况能够合并:a)检查目标的轮廓,b)检查之前的集合的轮廓.如何呢?核心做法都是,获取过去的符号,检查新符号,比较两个符号对应的点的集合的数量,选择少的,进行上面的步骤.
//   6.
//   对于draw与data的修改,都是比较简单的,直接获取当前点所在的集合,修改里面的点的符号为新符号.他不会去管背后的list如何合并.data直接修改?举个例子,我现在要fill这个集合,而如果该集合是少数,那么就回去检查这个集合的周围不是将变成的符号?应该是不是旧符号的,如果不是旧符号意味着什么?nothing,应该是正好是新符号的,因为这意味着两个集合再fill后需要合并.但是如果这样,list应该是正常的,只是data的修改将会失去修改目标,因为以前是旧符号的的集合需要需要全部换成新符号.现在通过合并会将旧符号的所在集合移动到新符号并与接壤的进行合并.
//    */
...
//   /*
//   1.
//   问题的重心再次变成,如何快速找到待合并的集合,如果在外面处理过直接用下面的merge处理在同一个符号下的list,那么似乎可行,但是我想避免重复遍历列表.首先在find_remove会处理任何的点一次,创建一个列表并放到新符号下面,那么这个状态需要逐个访问这些点并删除,所以我使用了data的修改和draw,但是这样我将不可以在获取prepare里面使用data,可以考虑的是,我的判断不用的?因为只要是当前的要更新的list就自动被更新成了新的code,那么就无法快速知道这个是自己还是接壤的如果可以在这之前完成合并人数组的获取那么可以接受
//   */
